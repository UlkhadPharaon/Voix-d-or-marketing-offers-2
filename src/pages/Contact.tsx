...
Merci !`;
    ...
    window.location.href = `https://wa.me/22656265915?text=${encodedMessage}`;
  ...
  if (isSubmitted) {
    return (
      <PageTransition className="min-h-screen bg-background/0 flex items-center justify-center pt-24 px-4 pb-12">
        ...
        >
          ...
          <h2 className="text-3xl font-heading font-bold text-foreground mb-4 uppercase tracking-[1px]">Brief Verrouillé.</h2>
          ...
          <Button onClick={() => window.location.href = '/'} variant="outline" className="text-foreground border-gray-700 hover:bg-gray-800">
            Retour à l'accueil
          </Button>
        </motion.div>
      </PageTransition>
      ...
      <div className="container mx-auto px-4 max-w-3xl">
        ...
        <div className="mb-12">
          <div className="flex justify-between mb-4">
            <span className={`text-[11px] font-bold uppercase tracking-[1px] ${step >= 1 ? 'text-primary' : 'text-muted-foreground'}`}>1. Établissement</span>
            <span className={`text-[11px] font-bold uppercase tracking-[1px] ${step >= 2 ? 'text-primary' : 'text-muted-foreground'}`}>2. Situation</span>
            <span className={`text-[11px] font-bold uppercase tracking-[1px] ${step >= 3 ? 'text-primary' : 'text-muted-foreground'}`}>3. Stratégie</span>
          </div>
          <div className="h-[2px] bg-foreground/10 flex overflow-hidden">
              ...
              <AnimatePresence mode="wait">
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-3">
                        <Label className="uppercase text-[11px] text-muted-foreground tracking-[1px]">Nom de l'établissement *</Label>
                        <Input required placeholder="Ex: Faso Restaurant" className="bg-foreground/[0.05] border-foreground/10 text-foreground rounded-[2px]" value={formData.company} onChange={e => setFormData({...formData, company: e.target.value})} />
                      </div>
                      <div className="space-y-3">
                        <Label className="uppercase text-[11px] text-muted-foreground tracking-[1px]">Votre prénom (le décideur) *</Label>
                        <Input required placeholder="Ex: Oumar Koné" className="bg-foreground/[0.05] border-foreground/10 text-foreground rounded-[2px]" value={formData.decisionMaker} onChange={e => setFormData({...formData, decisionMaker: e.target.value})} />
                      </div>
                    </div>
                    
                    <div className="space-y-3">
                      <Label className="uppercase text-[11px] text-muted-foreground tracking-[1px]">Numéro WhatsApp *</Label>
                      <Input required placeholder="+226 XX XX XX XX" type="tel" className="bg-foreground/[0.05] border-foreground/10 text-foreground rounded-[2px]" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} />
                    </div>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-3">
                        <Label className="uppercase text-[11px] text-muted-foreground tracking-[1px]">Secteur *</Label>
                        <Select required value={formData.sector} onValueChange={val => setFormData({...formData, sector: val})}>
                          <SelectTrigger className="bg-background border-foreground/10 rounded-[2px]">
                      ...
                      <div className="space-y-3">
                        <Label className="uppercase text-[11px] text-muted-foreground tracking-[1px]">Ville *</Label>
                    ...
                    <div className="space-y-3">
                      <Label className="uppercase text-[11px] text-muted-foreground tracking-[1px]">Lien de votre page Instagram ou Facebook (Optionnel)</Label>
                      <Input placeholder="https://instagram.com/votre_page" className="bg-foreground/[0.05] border-foreground/10 text-foreground rounded-[2px] transition-colors focus:border-primary" value={formData.socialLink} onChange={e => setFormData({...formData, socialLink: e.target.value})} />
                    </div>
                ...
                {step === 2 && (
                  ...
                  >
                    ...
                    <div className="space-y-3">
                      <Label className="uppercase text-[11px] text-muted-foreground tracking-[1px]">Qui gère vos publications actuellement ? *</Label>
                    ...
                    <div className="pt-6 flex justify-between">
                      <Button type="button" variant="outline" onClick={prevStep} className="border-foreground/10 text-foreground rounded-[2px] hover:bg-foreground/5 uppercase tracking-[1px] text-[12px] font-bold h-auto py-[12px]">
                        ← Retour
                      </Button>
                ...
                {step === 3 && (
                  ...
                  >
                    ...
                    <div className="space-y-3">
                      <Label className="uppercase text-[11px] text-muted-foreground tracking-[1px]">Décrivez votre produit/service phare en 1 phrase : *</Label>
                      <Input required placeholder="Ex: Nos pizzas au feu de bois..." className="bg-background border-foreground/10 text-foreground rounded-[2px]" value={formData.coreProduct} onChange={e => setFormData({...formData, coreProduct: e.target.value})} />
                    </div>

                    <div className="space-y-4 pt-4">
                      <Label className="uppercase text-[11px] text-muted-foreground tracking-[1px]">Sur quels réseaux voulez-vous cibler en priorité ? *</Label>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-2">
                        {['TikTok', 'Insta', 'WhatsApp', 'FB'].map(network => (
                          <div key={network} className="flex items-center space-x-2">
                            <Checkbox id={`net-${network}`} className="border-foreground/20 data-[state=checked]:bg-primary data-[state=checked]:text-black-deep" checked={formData.networks.includes(network)} onCheckedChange={(checked) => handleNetworkChange(network, checked as boolean)} />
                            <label htmlFor={`net-${network}`} className="text-[13px] leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 cursor-pointer text-foreground tracking-[0.5px]">
                              {network}
                            </label>
                          </div>
                        ))}
                    ...
                    <div className="space-y-3 pt-4">
                      <Label className="uppercase text-[11px] text-muted-foreground tracking-[1px]">Quel est l'objectif prioritaire de ces 30 prochains jours ? *</Label>
